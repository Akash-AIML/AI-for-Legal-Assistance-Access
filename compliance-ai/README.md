...
| **Speech STT & Neural TTS** | Web Speech API + GPT Neural Fallback (`tts-1` / `gpt-4o-mini-tts`) | `OPENAI_AUDIO_MODEL`, `OPENAI_BASE_URL` | Native browser Web Speech API for zero latency, with automatic backend neural TTS fallback for Linux/headless browsers lacking local voice engines (`synthesis-failed`). |
...
## ☁️ Automated Cloud Deployment CI/CD Workflows

The application includes production-grade GitHub Actions workflows for multi-cloud deployment:

### 1. Backend & Vector DB: Azure Container Apps (`.github/workflows/deploy-azure.yml`)
- Builds production container using `Dockerfile.backend`.
- Pushes container image to GitHub Container Registry (`ghcr.io`).
- Deploys container to **Azure Container Apps** (`compliance-ai-backend`).
- Mounts persistent storage volume for ChromaDB vector store & SQLite session database.

### 2. Frontend SPA: Vercel (`.github/workflows/deploy-vercel.yml`)
- Builds React 18 + Vite static application.
- Configures Vercel Edge proxy rewrites in `frontend/vercel.json` routing `/api/*` to the Azure Container App endpoint.
- Deploys globally with zero downtime.
...
| `OPENAI_API_KEY` | OpenAI / LLM Gateway API key |
| `OPENAI_BASE_URL` | Base URL for LLM provider gateway (`https://apidev.navigatelabsai.com/`) |
